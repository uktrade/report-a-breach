...
class TestUploadDocuments(conftest.PlaywrightTestBase):
    ...
    def test_incorrect_file_type(self):
        self.get_to_document_upload_page()
        self.upload_documents_page(self.page, files=data.INCORRECT_EXTENSION_FILE_TYPE)
        expect(self.page.get_by_role("heading", name="There is a problem")).to_be_visible()
        assert "incorrect_file_extension.sh cannot be uploaded" in self.page.locator(".govuk-error-summary__body").text_content()
