...
class TestCheckYourAnswersView:
    @patch("report_a_suspected_breach.views.views_end.get_missing_data")
    def test_dispatch_no_missing_data(self, mocked_get_missing_data, request_object):
        mocked_get_missing_data.return_value = {}
    ...
    @patch("report_a_suspected_breach.views.views_end.get_missing_data")
    def test_dispatch_missing_data(self, mocked_get_missing_data, request_object):
        mocked_get_missing_data.return_value = {"start": "reporter_professional_relationship"}
        view = CheckYourAnswersView()
        view.setup(request_object)
        response = view.dispatch(request_object)
        assert response.status_code == 302
        assert response.url == "/report/your-professional-relationship"
...
class TestDeclarationView:
    ...
    @patch("report_a_suspected_breach.views.views_end.get_missing_data")
    def test_dispatch_missing_data(self, mocked_get_missing_data, request_object):
        mocked_get_missing_data.return_value = {"email": "reporter_email"}
        view = DeclarationView()
        view.setup(request_object)
        response = view.dispatch(request_object)
        assert response.status_code == 302
        assert response.url == "/report/your-email-address"

    def test_form_valid_redirect_if_email_not_verified(self, request_object, rasb_client):
        ...
        reporter_object = ReporterEmailVerification.objects.create(
            reporter_session=request_session, email_verification_code="012345", verified=False
        )
